...
import { Building2, Bell, Shield, Database, Timer, Car, Pill, Wrench, Cylinder, BookOpen, Glasses, HelpCircle } from 'lucide-react';
...
export default function Configuracoes() {
  ...
  // Load SLA configs from localStorage (could be moved to database later)
  useEffect(() => {
    const saved = localStorage.getItem('sla_configs');
    if (saved) {
      try {
        setSlaConfigs(JSON.parse(saved));
      } catch {
        setSlaConfigs(defaultSlaConfigs);
      }
    }
  }, []);
  ...
  const handleSaveSla = () => {
    setSavingSla(true);
    try {
      localStorage.setItem('sla_configs', JSON.stringify(slaConfigs));
              ...
              <div className="rounded-xl border border-border bg-card p-6">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-[180px]">
                          ...
                          <span className="font-medium">{benefitTypeLabels[config.benefit_type]}</span>
                        </div>
                ...
                <div className="mt-6 flex justify-end">
                  <Button onClick={handleSaveSla} disabled={savingSla}>
                    {savingSla ? 'Salvando...' : 'Salvar Configurações de SLA'}
                  </Button>
                </div>
              </div>
            </TabsContent>
